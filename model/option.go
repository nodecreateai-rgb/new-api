...
func ensureDopioRMBPricing() {
	...
	for group, groupPrices := range modelGroupPrices {
		...
		if groupPrices["seedance-2.0-mini"] != 0.5 {
	...
	if err := ensureVobile2apiSeedanceRouting(); err != nil {
		common.SysLog("failed to enforce Seedance C1 gateway routing: " + err.Error())
	}
...
func ensureOAuth2APIRouting() error {
	...
	const neutralName = "OAuth2API"
	const legacyChannelName = "OAuth2 Auth"
	const groupsCSV = "default,vip,svip,vip1,vip2,vip3,vip6"
	groups := []string{"default", "vip", "svip", "vip1", "vip2", "vip3", "vip6"}
	...
	if common.UsingClickHouse {
		return ensureOAuth2APIRoutingClickHouse(neutralName, legacyChannelName, publicModel, groupsCSV, baseURL, key, groups, endpoint, description, vendorID)
	}
	...
	return ensureOAuth2APIMarketplaceModel(publicModel, endpoint, description, vendorID)
}

func ensureOAuth2APIRoutingClickHouse(neutralName, legacyChannelName, publicModel, groupsCSV, baseURL, key string, groups []string, endpoint, description string, vendorID int) error {
	var channel Channel
	err := DB.Where("name = ?", neutralName).First(&channel).Error
	if errors.Is(err, gorm.ErrRecordNotFound) {
		err = DB.Where("name = ?", legacyChannelName).First(&channel).Error
	...
	return ensureOAuth2APIMarketplaceModelClickHouse(publicModel, endpoint, description, vendorID)
}

func ensureOAuth2APIMarketplaceModel(publicModel, endpoint, description string, vendorID int) error {
	var meta Model
	err := DB.Unscoped().Where("model_name = ?", publicModel).First(&meta).Error
	if errors.Is(err, gorm.ErrRecordNotFound) {
		meta = Model{
			ModelName: publicModel, Description: description, Icon: "", Tags: "auth,oauth2",
		...
		"description": description, "icon": "", "tags": "auth,oauth2", "vendor_id": vendorID, "endpoints": endpoint,
		"status": 1, "sync_official": 0, "deleted_at": nil, "updated_time": common.GetTimestamp(),
	}).Error; err != nil {
		return err
	}
...
func ensureOAuth2APIMarketplaceModelClickHouse(publicModel, endpoint, description string, vendorID int) error {
	var meta Model
	err := DB.Unscoped().Where("model_name = ?", publicModel).First(&meta).Error
	if errors.Is(err, gorm.ErrRecordNotFound) {
		id := nextClickHouseTableID(DB, "models")
		...
		"description": description, "tags": "auth,oauth2", "vendor_id": vendorID, "endpoints": endpoint,
		"status": 1, "sync_official": 0, "deleted_at": nil, "updated_time": common.GetTimestamp(),
	}).Error; err != nil {
		return err
	}
...
func ensurePay2APIRouting() error {
	...
	key := strings.TrimSpace(os.Getenv("PAY2API_GATEWAY_KEY"))
	if key == "" {
		if keyFile := strings.TrimSpace(os.Getenv("PAY2API_GATEWAY_KEY_FILE")); keyFile != "" {
			if raw, err := os.ReadFile(keyFile); err == nil {
				key = strings.TrimSpace(string(raw))
			}
		}
...
func ensureVobile2apiSeedanceRouting() error {
	const neutralName = "Seedance C1 Video"
	const legacyChannelName = "Vobile Seedance Video"
	const modelsCSV = "seedance-2.0-c1,seedance-2.5-c1"
	const mappingJSON = `{"seedance-2.0-c1":"seedance-2.0-c1","seedance-2.5-c1":"seedance-2.5-c1"}`
	const groupsCSV = "default,vip,svip,vip1,vip2,vip3,vip6,vip8,vip9"
	baseURL := strings.TrimSpace(os.Getenv("VOBILE2API_BASE_URL"))
	if baseURL == "" {
		baseURL = "http://vobile2api:38691"
	}
	key := strings.TrimSpace(os.Getenv("VOBILE2API_GATEWAY_KEY"))
	if key == "" {
		if keyFile := strings.TrimSpace(os.Getenv("VOBILE2API_GATEWAY_KEY_FILE")); keyFile != "" {
			if raw, err := os.ReadFile(keyFile); err == nil {
				key = strings.TrimSpace(string(raw))
			}
		}
	...
	if key == "" {
		key = strings.TrimSpace(os.Getenv("VOBILE2API_API_KEY"))
	}

	publicModels := []string{"seedance-2.0-c1", "seedance-2.5-c1"}
	groups := []string{"default", "vip", "svip", "vip1", "vip2", "vip3", "vip6", "vip8", "vip9"}
	modelDescriptions := map[string]string{
		"seedance-2.0-c1": "Seedance 2.0 文生/图生视频（异步，¥2.5/次）",
		"seedance-2.5-c1": "Seedance 2.5 文生/图生视频（异步，¥4/次）",
	}
	endpoint := `{"openai-video":{"path":"/v1/videos","method":"POST"}}`

	if common.UsingClickHouse {
		return ensureVobile2apiSeedanceRoutingClickHouse(neutralName, legacyChannelName, modelsCSV, mappingJSON, groupsCSV, baseURL, key, publicModels, groups, modelDescriptions, endpoint)
	}

	if err := DB.Model(&Channel{}).Where("name = ?", legacyChannelName).Update("name", neutralName).Error; err != nil {
		return err
	}
...
func ensureVobile2apiSeedanceRoutingClickHouse(neutralName, legacyChannelName, modelsCSV, mappingJSON, groupsCSV, baseURL, key string, publicModels, groups []string, modelDescriptions map[string]string, endpoint string) error {
	if err := DB.Exec(`ALTER TABLE channels UPDATE name = ? WHERE name = ?`, neutralName, legacyChannelName).Error; err != nil {
		return err
	}
...
func ensureGetunikey2apiSeedanceRouting() error {
	const neutralName = "Seedance C2 Video"
	const modelsCSV = "seedance-2.5-c2"
	const mappingJSON = `{"seedance-2.5-c2":"bytedance/seedance-2.5"}`
	const groupsCSV = "default,vip,svip,vip1,vip2,vip3,vip6,vip8,vip9"
	...
	if common.UsingClickHouse {
		return ensureGetunikey2apiSeedanceRoutingClickHouse(neutralName, modelsCSV, mappingJSON, groupsCSV, baseURL, key, publicModels, groups, modelDescriptions, endpoint)
	}
...
func ensureGetunikey2apiSeedanceRoutingClickHouse(neutralName, modelsCSV, mappingJSON, groupsCSV, baseURL, key string, publicModels, groups []string, modelDescriptions map[string]string, endpoint string) error {
	var channel Channel
	err := DB.Where("name = ?", neutralName).First(&channel).Error
	if errors.Is(err, gorm.ErrRecordNotFound) {
		id := nextClickHouseTableID(DB, "channels")
